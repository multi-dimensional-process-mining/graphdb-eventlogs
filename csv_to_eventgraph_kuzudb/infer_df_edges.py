...
def getEntityTypes(conn: kuzu.Connection):
    qGetEntityTypes = f'''
        MATCH ( n : Entity) RETURN DISTINCT n.EntityType
        '''
    et_df = runQuery(conn, qGetEntityTypes).get_as_df()
    return et_df["n.EntityType"].to_list()
...
def getDerivedEntityTypes(conn: kuzu.Connection):
    qGetDerivedEntityTypes = f'''
        MATCH ( n : Entity ) -[:REL {{Type:"Reified"}}]-> (n2 : Entity )
        RETURN DISTINCT n.EntityType, n2.EntityType 
        '''
