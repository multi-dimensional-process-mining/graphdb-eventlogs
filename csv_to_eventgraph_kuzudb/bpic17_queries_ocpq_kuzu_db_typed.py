...
def main() -> None:
    ...
    response = runQuery(conn,'''
        MATCH (o:Application)
        OPTIONAL MATCH (e:Event) -[:CORR_Event_Application]-> (o)
        WHERE e.Activity = "A_Submitted"
        WITH o, COUNT(e) AS eventCount
        WHERE eventCount <> 1
        RETURN COUNT(o) AS violationCount
    ...
    response = runQuery(conn,'''
        MATCH (o:Application)
        OPTIONAL MATCH (e:Event {activity:"A_Submitted"}) -[:CORR_Event_Application]-> (o)
        WITH o, COUNT(e) AS eventCount
        WHERE eventCount <> 1
        RETURN COUNT(o) AS violationCount
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (e:Event {activity: "O_Created"})-[:CORR_Event_Offer]->(o:Offer)
        WHERE NOT EXISTS {
            MATCH (o) <-[:CORR_Event_Offer]- (e2:Event{activity: "O_Returned"}) WHERE e.timestamp < e2.timestamp
        }        
        RETURN COUNT(e) AS violationCount
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (e:Event)
        WHERE e.Activity = "O_Returned"
        OPTIONAL MATCH (e) -[:CORR_Event_Offer]->  (o:Offer)
        WITH e, COUNT(o) AS offerCount
        WHERE offerCount <> 1
        RETURN COUNT(e) AS violationCount
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (a:Application)<-[:CORR_Event_Application]-(e1:Event {activity:"A_Accepted"})
        OPTIONAL MATCH (a)<-[:REL_Offer_Application]-(o:Offer)<-[:CORR_Event_Offer]-(e2:Event {activity:"O_Accepted"})
        WHERE e1.timestamp <= e2.timestamp
        WITH a, COUNT(e2) AS e2_c, COUNT(o) AS o_c WHERE e2_c < 1 OR o_c < 1
        RETURN COUNT(a) as violationCount
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (a:Application)<-[:CORR_Event_Application]-(e1:Event {activity:"A_Accepted"})
        WHERE NOT EXISTS {
            MATCH (a)<-[:REL_Offer_Application]-(o:Offer)<-[:CORR_Event_Offer]-(e2:Event {activity:"O_Accepted"}) WHERE e1.timestamp <= e2.timestamp
        }
        RETURN COUNT(a) as violationCount
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (o1:Application) <-[:CORR_Event_Application]- (e1:Event {activity: 'A_Accepted'}) -[:CORR_Event_Resource]-> (o2:Resource)
        OPTIONAL MATCH (e2:Event {activity: 'O_Created'}) -[:CORR_Event_Offer]-> (o3:Offer) -[:REL_Offer_Application]-> (o1)
        WHERE NOT (e2) -[:CORR_Event_Resource]-> (o2)
        WITH o1, COUNT(e2) as e2_c, COUNT(o3) AS o3_c
        WHERE e2_c >= 1 OR o3_c >= 1
        RETURN COUNT(*) AS violationCount
                            ''')
    ...
    print("Running Q5 modified #1")
    response = runQuery(conn,'''
        MATCH (o1:Application) <-[:CORR_Event_Application]- (e1:Event {activity: 'A_Accepted'}) -[:CORR_Event_Resource]-> (o2:Resource)
        OPTIONAL MATCH (o1) <-[:REL_Offer_Application]- (o3:Offer) <-[:CORR_Event_Offer]- (e2:Event {activity: 'O_Created'}) -[:CORR_Event_Resource]-> (o4:Resource) WHERE o2 <> o4
    ...
    response = runQuery(conn,'''
        MATCH (o1:Application) <-[:CORR_Event_Application]- (e1:Event {activity: 'A_Accepted'})
        OPTIONAL MATCH (o1) <-[:REL_Offer_Application]- (o3:Offer) <-[:CORR_Event_Offer]- (e2:Event {activity: 'O_Created'}) WHERE e1.resource <> e2.resource
        WITH o1, COUNT(e2) as e2_c
        WHERE e2_c >= 1
        RETURN COUNT(o1) AS violationCount
                            ''')
    ...
    print("Running Q5 modified #3")
    response = runQuery(conn,'''
        MATCH (o1:Application) <-[:CORR_Event_Application]- (e1:Event {activity: 'A_Accepted'})
        WHERE EXISTS { 
            MATCH (o1) <-[:REL_Offer_Application]- (o3:Offer) <-[:CORR_Event_Offer]- (e2:Event {activity: 'O_Created'}) WHERE e1.resource <> e2.resource
        }
        RETURN COUNT(o1) AS violationCount
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (e1:Event {activity : "O_Created"}) -[:CORR_Event_Offer] -> (o:Offer) <-[:CORR_Event_Offer]- (e2:Event {activity : "O_Accepted"})
    ...
    response = runQuery(conn,'''
        MATCH (e2:Event {activity: 'O_Created'}) -[:CORR_Event_Offer]-> (o2:Offer) -[:REL_Offer_Application]-> (o1:Application)
        MATCH (o1)  <-[:REL_Offer_Application]- (o3:Offer) <-[:CORR_Event_Offer]- (e3:Event {activity: 'O_Created'})
        RETURN COUNT(*)
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (e2:Event {activity: 'O_Created'}) -[:CORR_Event_Offer]-> (o2:Offer) -[:REL_Offer_Application]-> (o1:Application) <-[:REL_Offer_Application]- (o3:Offer) <-[:CORR_Event_Offer]- (e3:Event {activity: 'O_Created'})
        RETURN COUNT(*)
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (o2:Offer) -[:REL_Offer_Application]-> (o1:Application) <-[:REL_Offer_Application]- (o3:Offer)
        RETURN COUNT(*)
                            ''')
    ...
    response = runQuery(conn,'''
        MATCH (e2:Event {activity: 'O_Created'}) -[:CORR_Event_Offer]-> (o2:Offer) -[:REL_Offer_Application]-> (o1:Application) <-[:REL_Offer_Application]- (o3:Offer) <-[:CORR_Event_Offer]- (e3:Event {activity: 'O_Created'}) WHERE o2 <> o3
        RETURN COUNT(*)
                            ''')
    ...
    print("Running Q7 modified to check for applications with distinct offers #2")
    response = runQuery(conn,'''
        MATCH (o2:Offer) -[:REL_Offer_Application]-> (o1:Application) <-[:REL_Offer_Application]- (o3:Offer) WHERE o2 <> o3
        RETURN COUNT(*)
                            ''')
