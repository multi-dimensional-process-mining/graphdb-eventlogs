...
# 20 randomly selected cases showing variety of behavior
cases = ['Application_2045572635', 
...
case_selector_e2 = "e2.case IN "+str(cases)
...
def getResourcesDF(tx, dot, edge_width):
    q = f'''
        ...
        WHERE {case_selector} AND {case_selector_e2} 
        return e1,df,e2,r
        '''
    for record in tx.run(q):
...
with driver.session() as session:
    ...
    session.read_transaction(getResourcesDF, dot, 5)
