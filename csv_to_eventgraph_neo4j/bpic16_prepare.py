...
# config
sample = False
inputpath = '..\\data\\BPIC16\\'
path_to_neo4j_import_directory = '..\\data\\BPIC16\\prepared\\'  # where prepared files will be stored


################## data prep ##################  

def CreateBPI16(inputpath, outputpath, fileName, sample):
    clicksLog = pd.read_csv(os.path.realpath(inputpath + 'BPI2016_Clicks_Logged_In.csv'), keep_default_na=True, sep=';',
                            encoding='latin1')
    complaints = pd.read_csv(os.path.realpath(inputpath + 'BPI2016_Complaints.csv'), keep_default_na=True, sep=';',
                             encoding='latin1')
    questions = pd.read_csv(os.path.realpath(inputpath + 'BPI2016_Questions.csv'), keep_default_na=True, sep=';',
                            encoding='latin1')
    messages = pd.read_csv(os.path.realpath(inputpath + 'BPI2016_Werkmap_Messages.csv'), keep_default_na=True, sep=';',
                           encoding='latin1')

    if (sample):
        sampleIds = [2026796, 2223803, 2023026, 114939, 2011721, 2022933, 919259, 2079086, 466152, 2057965, 1039204,
    ...
    for case in sampleIds:
        for index, row in csvLog[
            csvLog.CustomerID == case].iterrows():  # second iteration through the cases for adding data
    ...
    logSamples['ContactDate'] = logSamples['ContactDate'].map(
    ...
    logSamples['EventDateTime'] = logSamples['EventDateTime'].map(
        lambda x: x.strftime('%Y-%m-%dT%H:%M:%S.%f')[0:-3] + '+0100')
    ...
    logSamples = logSamples.drop(columns=list(logSamples.columns[range(-1, -10, -1)]))
